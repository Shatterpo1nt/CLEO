...
echo "  → Une fenêtre va s'ouvrir dans Chrome"
echo "  → Autorise l'accès, puis reviens ici"
echo ""
...
if ! echo "$STATUS" | grep -qi "Email:"; then
  echo ""
  echo "❌ Login non détecté. Vérifie que tu as bien cliqué sur 'Authorize' dans Chrome."
...
# Find the site
echo "🔍 Recherche du site merci-cleo..."
SITE_INFO=$(netlify sites:list 2>&1)
echo "$SITE_INFO" | grep -i "merci\|cleo"
SITE_ID=$(netlify api getSiteByName --data '{"name":"merci-cleo"}' 2>/dev/null | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{console.log(JSON.parse(d).id||'')}catch(e){}})" 2>/dev/null)

if [ -z "$SITE_ID" ]; then
  # Try by slug
  SITE_ID=$(netlify env:list --site merci-cleo 2>/dev/null && echo "merci-cleo" || echo "")
  SITE_ARG="merci-cleo"
else
  SITE_ARG="$SITE_ID"
fi

echo "  Site: ${SITE_ARG:-merci-cleo}"
echo ""
...
netlify env:list --site merci-cleo 2>&1 | head -30
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "⚠️  STRIPE_WEBHOOK_SECRET à ajouter après création du webhook."
echo ""
read -rp "Entrée pour fermer..."
